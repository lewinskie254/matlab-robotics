...
real_robot1(:,1) = [-20; 20; 0];     % x (m) ; y (m) ; rotation (rad)
real_robot2(:,1) = [-10; 10; 0];     % x (m) ; y (m) ; rotation (rad)
...
meas_landmark1(:,:,1) = Measurement(real_robot1(:,1),real_landmarks+randn(2,1));
meas_landmark2(:,:,1) = Measurement(real_robot2(:,1),real_landmarks+randn(2,1));

index_seen = zeros(3,num_landmarks,timesteps); % Index for landmark has been discovered at least once (1: robot1, 2: robot2, 3: Fusion)
index_fov = zeros(3,num_landmarks,timesteps);  % Index for landmark is close enough (1: robot1, 2: robot2, 3: Fusion)
for t = 2:timesteps
    % Move the robot
    ...
    for l = 1:num_landmarks
        if sqrt(sum((real_robot1(1:2,t)-real_landmarks(:,l)).^2))<max_read_distance
            meas_landmark1(:,l,t) = Measurement(real_robot1(:,t),real_landmarks(:,l)+randn(2,1));
            index_seen(1,l,t:end) = 1;
            index_fov(1,l,t) = 1;
        end
        if sqrt(sum((real_robot2(1:2,t)-real_landmarks(:,l)).^2))<max_read_distance
            meas_landmark2(:,l,t) = Measurement(real_robot2(:,t),real_landmarks(:,l)+randn(2,1));
...
%% EKF parameters %% parâmetros que podem ser mudados (principalmente ! e P0)
Q = diag(process_variance);                                   % State covariance
R = diag(measurement_variance);                             % True Measurement covariance (not know in reality)
Rm = diag([measurement_variance;measurement_variance]);     % Measurement covariance for measurement fusion
P0 = diag([1 1]); %first = 100 second = 10; third= 0.1; fourth= 0.01;

%% Particle filter parameters %% pode ser mudado tbm (era 100)
num_particles = 100;

%% Initial estimates
initial_landmarks = real_landmarks + [1;1].*randn(2,num_landmarks); 
initial_robot1 = real_robot1(:,1) + [1;1;0].*randn(3,1); 
initial_robot2 = real_robot2(:,1) + [1;1;0].*randn(3,1); 
