function particles_SF = Fusion_covariance(particles_robot1, particles_robot2, particles_SF, index_fov)
...
for l = 1:num_landmarks
    for p = 1:num_particles
        if index_fov(3,l) % seen by both robots    
            ...
            inv_A = inv(particles_robot1(p).landmarks(l).P);
            inv_B = inv(particles_robot2(p).landmarks(l).P);
